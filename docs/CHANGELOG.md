...
### Changed
- Audit cluster moved to `@a-eyes/security` (#26 phase 2): `audit-signing.ts`, `audit-log.ts`, `verify-logs.ts` and their tests. `server.ts` imports `writeAuditEntry` from the workspace package; `src/index.ts` imports `verifyLogs` from it. No behavior change; `console.log` output of `verifyLogs` preserved (post-migration follow-up to return structured `VerifyResult[]` instead). `packages/security/README.md` stub added.
- `RateLimiter` moved from `src/rate-limiter.ts` to `@a-eyes/security` (#26 phase 1). Server imports `RateLimiter` from the workspace package; behavior unchanged.
...
- `dpi_mode` parameter for `capture` and `query` tools — resolves #6. `native` (default) returns raw pixel resolution; `logical` downscales to match visible UI size on HiDPI displays. Uses `GetDpiForWindow` (per-window) with `GetDeviceCaps(LOGPIXELSX)` fallback; composes with `max_width` (DPI scaling runs first). Substantial payload reduction on 150%/200% displays.
- `mode` parameter for `see` tool — resolves #12. `full` (default) returns screenshot + UI element tree + visible text; `text` returns screenshot + visible text only, skipping the element list for a smaller payload. Both modes walk the element tree internally to harvest nested text.
- `copy_to_clipboard` parameter for `capture` tool — resolves #14. When true, copies the captured image to the Windows clipboard. Clipboard write runs in TypeScript AFTER redaction is applied, so any redaction rule matching the window also applies to the clipboard copy. Rejected in frontmost mode and when matching redaction rules are detected pre-capture (defense-in-depth). Uses extended `clipboard.ps1` with `write-image` action via stdin.
- `watch` MCP tool — resolves #13. Polls a window until its content changes (or timeout). Parameters: `window_title` / `process_name` (required, frontmost mode rejected), `mode` (window|screen), `poll_interval_ms` (100–60000, default 1000), `timeout_seconds` (1–300, default 30). Returns the frame at change-moment plus `polls` and `elapsed_ms`. Pure TypeScript loop over existing `captureWindow()`, SHA-256 hash diff. DoS mitigation: hard Zod bounds, entry-gate rate limit check + per-poll `record()`+`isAllowed()` (slots consumed match actual polls, unused budget remains available). Allowlist re-verified against each poll's actual window metadata to close TOCTOU gaps. New `tryReserve(n)` helper on `RateLimiter` for future reservation use cases.
...
### Security
- `watch` tool intentionally skips content-redaction during polling (redaction would break hash stability across frames). Callers that need redacted output from a watched window should run `capture` on the returned window after the watch completes. Documented limitation; reconsider for GA.
